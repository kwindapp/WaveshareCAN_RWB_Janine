 ...
 * FLAGS / CAN
 ***************************************************/
bool testMode = true;   // true = simulate values
...
void aim_decode_packet_to_ecu(const uint8_t *p) {
  ...
  // Battery (12–13) – leave your scaling
  ...
  // Others available if needed later:
  // float map_mbar = U16BE_bytes(&p[16]) * 1.0f;
  // float airC     = S16BE_bytes(&p[18]) * 0.1f;
  // float fuelC    = S16BE_bytes(&p[24]) * 0.1f;
}
...
void decodeAIM_770(const twai_message_t &m) {
  ...
  for (int i = 0; i < m.data_length_code; ++i) {
    ...
    if (aim_ring[idx_prev2] == 0xFC &&
      ...
      // idx_prev2 is physical index of logical byte31
      ...
      // Optional debug: first few packets
      if (can_rx_count < 5) {
...
void create_sim_ui() {
  ...
  lv_obj_align(label_title, LV_ALIGN_TOP_MID, -100, 60);
  ...
  lv_obj_align(label_rpm, LV_ALIGN_CENTER, -100, -120);
  ...
  lv_obj_align(btn_left, LV_ALIGN_CENTER, -300, -120);
  ...
  lv_obj_align(btn_right, LV_ALIGN_CENTER, 120, -120);
  ...
  lv_obj_align(label_scale, LV_ALIGN_CENTER, -100, -260);

  /********* RPM BAR *********/
  rpm_bar = lv_bar_create(scr);
  lv_bar_set_range(rpm_bar, 0, 800);   // 0–8000 rpm -> /10
  lv_bar_set_value(rpm_bar, 0, LV_ANIM_OFF);
  lv_obj_set_size(rpm_bar, 700, 25);
  lv_obj_align(rpm_bar, LV_ALIGN_CENTER, -100, -50);
  ...
  lv_obj_align(label_row1, LV_ALIGN_BOTTOM_MID, -100, -230);
  ...
  lv_obj_align(label_row2, LV_ALIGN_BOTTOM_MID, -100, -160);
  ...
  lv_obj_align(label_row3, LV_ALIGN_BOTTOM_MID, -100, -130);
}
...
void ui_update_from_ecu() {
  ...
  // RPM label
  uint16_t rpm_display = ecu.rpm;
  if (rpm_display > MAX_RPM) rpm_display = MAX_RPM;
  snprintf(buf, sizeof(buf), "%u", rpm_display);
  lv_label_set_text(label_rpm, buf);

  // RPM bar
  int bar = rpm_display / 10;  // 0..8000 -> 0..800
  if (bar < 0)   bar = 0;
  if (bar > 800) bar = 800;
  lv_bar_set_value(rpm_bar, bar, LV_ANIM_OFF);
